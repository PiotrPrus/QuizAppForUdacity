...
public class QuizPageThree extends AppCompatActivity {
    ...
    public void goToPageForth(View view) {

        if (isProperAnswerSelected()) {
            quizScore++;
        }
        Log.d("quizResult", "Result after third question is :" + quizScore);
        Intent startForthQuestion = new Intent(this, QuizPageFour.class);
        startForthQuestion.putExtra(String.valueOf(R.string.saved_quiz_score), quizScore);
        startActivity(startForthQuestion);
        finish();
    }

    private boolean isProperAnswerSelected(){
        return (radioButton3.isChecked());
    }
}
